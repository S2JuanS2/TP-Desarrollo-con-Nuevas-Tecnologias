...
class ArticuloController {
    ...
    //muestra la lista de artículos disponibles. Si un cliente está autenticado en la sesión y tiene un pedido en curso, muestra la información del pedido actual. Si no hay ningún pedido en curso para el cliente autenticado, muestra la lista de artículos disponibles para su compra. Si no hay ningún cliente autenticado en la sesión, muestra una vista de "registro fallido" para indicar que el usuario no tiene acceso sin autenticación.
    ...
    //permite al administrador agregar un nuevo artículo a la base de datos. Redirige al administrador de vuelta a la vista de administración para que pueda seguir gestionando otros aspectos de la aplicación.
    ...
    //permite al administrador reducir el stock de un artículo específico en la base de datos. Antes de reducir el stock, verifica que el stock actual sea mayor que 0 para evitar valores negativos. Redirige al administrador de vuelta a la vista de administración para que pueda seguir gestionando otros aspectos de la aplicación.
    def reducirStock(){
